...
class FileUploader
{
    ...
    private function validate($item = null)
    {
        if (null == $item) {
            ...
            if ($this->options['required'] && 'post' == strtolower($_SERVER['REQUEST_METHOD']) && $this->field['count'] + count($this->options['files']) == 0) {
                return $this->codeToMessage('required_and_no_file');
            }
            if (($this->options['limit'] && $this->field['count'] + count($this->options['files']) > $this->options['limit']) || (0 != $ini[3] && ($this->field['count']) > $ini[3])) {
    ...
    public static function resize($filename, $width = null, $height = null, $destination = null, $crop = false, $quality = 95, $rotation = 0)
    {
        ...
        $hasCrop = is_array($crop) || true == $crop;
        $hasResizing = $width || $height;

        if (!$hasRotation && !$hasCrop && !$hasResizing && (!isset($exif['Orientation']) || 1 == $exif['Orientaiton']) && $filename == $destination) {
            return;
        }
    ...
    private function uploadFiles()
    {
        ...
        $chunk = isset($_POST['_chunkedd']) && 1 == count($this->field['input']['name']) ? json_decode(stripslashes($_POST['_chunkedd']), true) : false;
        ...
        if ($this->field['input']) {
            ...
            if ($lastPHPError && E_WARNING == $lastPHPError['type'] && 0 == $lastPHPError['line']) {
                $errorMessage = null;
            ...
            if ($this->options['required'] && 'post' == strtolower($_SERVER['REQUEST_METHOD'])) {
                $data['hasWarnings'] = true;
    ...
    protected function editFiles()
    {
        ...
        foreach ($this->getFileList() as $key => $item) {
            ...
            if (isset($item['uploaded']) && isset($_POST['_editorr']) && $this->isJSON(stripcslashes($_POST['_editorr'])) && 1 == count($this->field['input']['name'])) {
                $item['editor'] = json_decode(stripslashes($_POST['_editorr']), true);
            }

            // edit file
            if (file_exists($file) && 0 === strpos($item['type'], 'image/')) {
    ...
    private function generateFilename($conf, $item, $skip_replace_check = false)
    {
        ...
        $forceExtension = isset($conf[2]) && true == $conf[2];
        ...
        if (!$this->options['replace'] && !$skip_replace_check) {
            ...
            while (file_exists($this->options['uploadDir'].$string)) {
                $item['title'] = $title." ({$i})";
                $conf[0] = 'auto' == $type || 'name' == $type || false !== strpos($string, '{random}') ? $type : $type." ({$i})";
    ...
    public static function clean_chunked_files($directory, $time = '-1 hour')
    {
        ...
        foreach ($files as $key => $name) {
            $file = $directory.$name;
            if (0 === strpos($name, '.unconfirmed_') && filemtime($file) < strtotime($time)) {
                unlink($file);
            }
        }
    }
