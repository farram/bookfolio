...
import ButtonNeverSuggest from "./ButtonNeverSuggest.vue";

export default {
  components: {
    "button-follow": ButtonFollow,
    "button-never-suggest": ButtonNeverSuggest,
  },
  data: function () {
    return {
      page: 1,
  ...
  computed: {
    likesAllType() {
      return this.newsType.length === this.typeList.length;
    },
    likesSomeType() {
      return this.newsType.length > 0 && !this.likesAllType;
    },
    icon() {
      if (this.likesAllType) return "mdi-close-box";
      if (this.likesSomeType) return "mdi-minus-box";
  ...
  methods: {
    ...
    infiniteHandler($state) {
      this.$http
        .get(
          Routing.generate("json_annuaire_suggested", {
            page: this.page,
          }),
          {
            params: {
    ...
    toggle() {
      this.$nextTick(() => {
        if (this.likesAllType) {
          this.newsType = [];
        } else {
          this.newsType = this.typeList.slice();
        }
        this.disabled = true;
      });
    },
